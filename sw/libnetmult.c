...
int main()
{
   ...
   uint16_t ports_to_drop = malloc(num_pkts*sizeof(uint16_t));
   ...
   for(i=0;i<num_pkts;i++){
      src_prt = ports_to_drop[num_pkt-i-1];
   ...
   /*--------------ACK----------------*/
   for(i=0;i<num_pkts;i++){
      dst_prt = ports_to_drop[num_pkt-i-1];
      src_prt = ports_to_drop[i]; 
      seqn=i*(LIBNET_TCP_H+payload_s+1);
